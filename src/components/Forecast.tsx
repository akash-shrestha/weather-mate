...
const Forecast = ({ data, setForecast, setCity, setTerm }: Props) => {
    ...
    <div className="w-full md:max-w-[500px] py-4 md:py-4 md:px-10 lg:px-24 h-full lg:h-auto bg-white bg-opacity-20 backdrop-blur-ls rounded drop-shadow-lg">
      <div className="mx-auto w-[300px]">
        <div className="flex items-center justify-center mb-5">
          <button
            className="rounded-full border-2 border-zinc-200 hover:border-zinc-100 hover:text-zinc-200  text-zinc-100 px-2 py-1 cursor-pointer"
            onClick={onSubmit}
          >
            Change location
          </button>
        ...
        <section className="flex overflow-x-scroll mt-4 pb-2 mb-5">
          {data.list.map((item) => (
            ...
            >
              <p className="text-sm">
                {item.dt === 0 ? 'Now' : new Date(item.dt * 1000).getHours()}
              </p>
              <img
        ...
        <section className="flex flex-wrap justify-between text-zinc-700 overflow-y: auto">
